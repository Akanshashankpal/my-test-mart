...
export class BillingService {
  ...
  async getBills(filters: any, pagination: { page: number; limit: number }) {
      ...
      filteredBills = filteredBills.filter(bill => 
        filters.$or.some((condition: any) => {
          ...
          if (condition['customer.name']) {
            return bill.customer.name.toLowerCase().includes(condition['customer.name'].$regex.toLowerCase());
          }
          if (condition['customer.phone']) {
            return bill.customer.phone.includes(condition['customer.phone'].$regex);
          }
          return false;
        })
      );
    ...
    if (filters.paymentStatus) {
      filteredBills = filteredBills.filter(bill => bill.paymentStatus === filters.paymentStatus);
    }
  ...
  async getDashboardStats() {
    ...
    const totalRevenue = this.bills.reduce((sum, bill) => sum + bill.finalAmount, 0);
    const todaysRevenue = todaysBills.reduce((sum, bill) => sum + bill.finalAmount, 0);
    const monthsRevenue = monthsBills.reduce((sum, bill) => sum + bill.finalAmount, 0);

    const uniqueCustomers = new Set(this.bills.map(bill => bill.customer.id)).size;
    const todaysCustomers = new Set(todaysBills.map(bill => bill.customer.id)).size;
    ...
    return {
      ...
      paymentStatusBreakdown: {
        paid: this.bills.filter(b => b.paymentStatus === "Paid").length,
        pending: this.bills.filter(b => b.paymentStatus === "Pending").length,
        overdue: this.bills.filter(b => b.paymentStatus === "Overdue").length,
      },
      billTypeBreakdown: {
        gst: this.bills.filter(b => b.billType === "GST").length,
        nonGst: this.bills.filter(b => b.billType === "Non-GST").length,
      ...
      filteredReturns = filteredReturns.filter(ret => 
        filters.$or.some((condition: any) => {
          if (condition.returnNumber) {
            return ret.returnNumber.toLowerCase().includes(condition.returnNumber.$regex.toLowerCase());
          }
          if (condition.originalBillNumber) {
            return ret.originalBillNumber.toLowerCase().includes(condition.originalBillNumber.$regex.toLowerCase());
          }
          if (condition['customer.name']) {
            return ret.customer.name.toLowerCase().includes(condition['customer.name'].$regex.toLowerCase());
          }
          return false;
        })
      );
  ...
  async updateSalesReturnStatus(id: string, status: string): Promise<SalesReturn | null> {
    ...
    this.salesReturns[index].status = status as any;
  ...
  async generatePDF(bill: Bill): Promise<Buffer> {
    ...
    const pdfContent = `
      ...
      Items:
      ${bill.items.map(item => 
        `${item.productName} x ${item.quantity} @ ₹${item.rate} = ₹${item.totalAmount}`
      ).join('\n')}
